...
class Food:
    ...
    def _fallback_embed(self, data: dict, image_url: str) -> discord.Embed:
        ...
        emb.add_field(name="Kalorien", value=f"≈ **{int(self._num(data.get('kcal')))} kcal**")
        emb.add_field(name="Eiweiß", value=f"{self._num(data.get('protein_g')):g} g")
...
# Modul-Aliase: die bisherige Modul-Schnittstelle bleibt erhalten.
HANDLED = Food.HANDLED
CHANNEL_ID = Food.CHANNEL_ID
