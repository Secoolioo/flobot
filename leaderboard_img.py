...
class LeaderboardImg:
    ...
    def _glyph_ok(self, font, ch: str) -> bool:
        ...
        fid = id(font)
        ck = (fid, ch)
        if ck in self._glyph_cache:
            return self._glyph_cache[ck]
        notdef = self._notdef_cache.get(fid)
        if notdef is None:
            try:
                notdef = self._chbytes(font, "￿")   # Nicht-Zeichen -> garantiert Tofu
            except Exception:  # noqa: BLE001
                notdef = b""
            self._notdef_cache[fid] = notdef
        try:
            b = self._chbytes(font, ch)
    ...
    def _draw_avatar(self, img, d, row: dict, name: str, cy: int, ring) -> None:
        ...
        av = row.get("avatar")
        if av:
            circ = self._avatar_circle(av, _AVA_D)
        if circ is not None:
    ...
    def render_png(self, rows: list[dict], *, title: str = "FLO  LEADERBOARD",
        ...
        if not _PIL_OK or not rows:
            return None
        try:
            return self._render(rows, title, subtitle)
    ...
    def _render(self, rows: list[dict], title: str, subtitle: str) -> bytes:
        ...
        self._draw_header(d, title, subtitle)

        f_name = self._font(23, bold=True)
        f_meta = self._font(15)
        f_rank = self._font(22, bold=True)
        f_val = self._font(16, bold=True)
        ...
        for i, r in enumerate(rows):
            ...
            safe = self._safe_name(f_name, r.get("name", ""), fallback=f"Spieler #{i + 1}")

            # --- Profilbild (oder Initial-Platzhalter) mit Rang-Ring ---
            self._draw_avatar(img, d, r, safe, cy, rc or _BORDER)

            # --- Name + Meta-Zeile (Level · Coins · Titel) ---
            name = self._truncate(d, safe, f_name, _NAME_W - (26 if (r.get("throne") or r.get("crown")) else 0))
    ...
    def _draw_header(self, d, title: str, subtitle: str) -> None:
        ...
        tw_title = d.textlength(title, font=self._font(30, bold=True))
...
instance = LeaderboardImg()

# Modul-Aliase: diese Namen muessen weiter wie bisher funktionieren.
is_available = instance.is_available
render_png = instance.render_png
