...
# --- Synthese-Helfer -------------------------------------------------------
def _silence(dauer: float) -> list[float]:
    return [0.0] * int(RATE * dauer)


def _env(n: int, attack: float = 0.005, release: float = 0.05) -> list[float]:
...
def _smooth(samples: list[float], breite: int = 3) -> list[float]:
...
def _ton(dauer: float, freq, *, wellen=("saw",), vibrato: float = 0.0,
...
def _mix_at(ziel: list[float], teil: list[float], start_s: float,
            gain: float = 1.0) -> None:
    o = int(RATE * start_s)
...
def _badumtss() -> list[float]:
    out: list[float] = []

    def tom(f0: float, f1: float, dauer: float) -> list[float]:
        return [s * d for s, d in zip(
            _ton(dauer, lambda t, a=f0, b=f1: a + (b - a) * (t * 4), wellen=("sine",)),
            _exp_decay(int(RATE * dauer), 0.09))]

    _mix_at(out, tom(190, 120, 0.22), 0.0)
    _mix_at(out, tom(150, 95, 0.22), 0.24)
    ...
    dec = _exp_decay(n - 1, 0.35)
    _mix_at(out, [h * d for h, d in zip(hell, dec)], 0.48, gain=0.8)
    return out


def _trompete() -> list[float]:
    """Traurige Trompete: waaah waaah waaah waaaaaah (faellt am Ende ab)."""
    out: list[float] = []
    t0 = 0.0
    for f, dauer in ((233.0, 0.38), (220.0, 0.38), (208.0, 0.38)):
        ton = _ton(dauer, f, wellen=("saw", "square"), vibrato=0.02,
                   vib_hz=5.5, attack=0.03, release=0.08)
        _mix_at(out, _smooth(ton, 4), t0)
        t0 += dauer + 0.06
    # letzter Ton: lang, tiefes Vibrato, sackt am Schluss ab
    dauer = 1.3
    ton = _ton(dauer, lambda t: 196.0 - (26.0 * max(0.0, t - 0.7) / 0.6),
...
def _applaus() -> list[float]:
    ...
    while t < 2.0:
        ...
        if random.random() < max(0.15, dichte):
            ...
            dec = _exp_decay(n - 1, 0.004)
            _mix_at(out, [k * d for k, d in zip(klatsch, dec)], t,
                    gain=random.uniform(0.4, 1.0))
        t += random.uniform(0.008, 0.03)
    return out


def _buzzer() -> list[float]:
    """Falsche-Antwort-Buzzer: fies, rau, unmissverstaendlich."""
    a = _ton(0.85, 112.0, wellen=("square", "saw"), attack=0.004, release=0.1)
    b = _ton(0.85, 89.0, wellen=("square",), attack=0.004, release=0.1)
...
def _tada() -> list[float]:
    """Fanfare: C-E-G hoch, dann Akkord mit Glitzer."""
    out: list[float] = []
    for i, f in enumerate((523.25, 659.26, 783.99)):
        ton = _ton(0.14, f, wellen=("saw", "sine"), attack=0.01, release=0.04)
        _mix_at(out, _smooth(ton, 3), i * 0.10, gain=0.8)
    akkord: list[float] = []
    for f in (523.25, 659.26, 783.99, 1046.5):
        ton = _ton(1.3, f, wellen=("saw", "sine"), vibrato=0.012, vib_hz=5.5,
                   attack=0.02, release=0.5)
        if not akkord:
            akkord = [0.0] * len(ton)
        for j, s in enumerate(ton):
            akkord[j] += s * 0.4
    _mix_at(out, _smooth(akkord, 3), 0.30)
    return out


def _pups() -> list[float]:
    ...
    for i in range(n):
        ...
        s = _saw(phase) * 0.8 + random.uniform(-1, 1) * 0.18
...
def _pew() -> list[float]:
    ...
    def pew() -> list[float]:
        return [s * d for s, d in zip(
            _ton(0.22, lambda t: 1400.0 * math.exp(-t * 10.0) + 160.0,
                 wellen=("square", "sine"), attack=0.002),
            _exp_decay(int(RATE * 0.22), 0.1))]
...
PACK = {
    "airhorn": _airhorn, "boom": _boom, "badumtss": _badumtss,
    "trompete": _trompete, "applaus": _applaus, "buzzer": _buzzer,
    "ding": _ding, "tada": _tada, "pups": _pups, "pew": _pew, "gong": _gong,
}


def ensure_pack(sounds_dir: Path) -> int:
    ...
    neu = 0
    for name, bauen in PACK.items():
        if name in vorhanden:
            continue
        random.seed(name)                # reproduzierbar, unabhaengig vom Start
        _write(sounds_dir / f"{name}.wav", bauen())
        neu += 1
    return neu
